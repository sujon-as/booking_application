    ...
    <section class="content">
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Add Working Days</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form action="{{route('workingdays.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="name"
                                    placeholder="Working Days" required="" value="{{old('name')}}">
                                @error('name')
                                ...
                                <select class="form-control select2bs4" name="sort_order" id="sort_order" required="">
                                    ...
                                    @if(count($availableSortOrders) > 0)
                                        @foreach ($availableSortOrders as $availableSortOrder)
                                            <option value="{{ $availableSortOrder }}">{{ $availableSortOrder }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('sort_order')
